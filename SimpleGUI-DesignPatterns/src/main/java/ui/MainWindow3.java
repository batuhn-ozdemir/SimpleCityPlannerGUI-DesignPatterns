...
public class MainWindow3 extends Application implements WeatherObserver {

    private ListView<City> cityListView = new ListView<>();
    private ListView<String> weatherCityBox = new ListView<>();
    ...
    public void start(Stage stage) {
        ...
        TitledPane cityPane = new TitledPane("Cities & Filters", cityBox);
        cityPane.setCollapsible(false);

        // Weather-Based City List Panel
        VBox weatherBox = new VBox(10,
                new Label("Cities Matching Weather Condition:"),
                weatherCityBox
        );
        weatherBox.setPadding(new Insets(10));
        TitledPane weatherPane = new TitledPane("Weather-Based City List", weatherBox);
        weatherPane.setCollapsible(false);

        // Activity Planner Panel
        ...
        // Left Pane (All Cities + Weather Box + Planner)
        VBox leftPane = new VBox(15, cityPane, weatherPane, plannerPane);
        ...
        // Initialize data and weather updates
        updateCityList();
        updateCharts();
    ...
    private void updateCityList() {
        ...
        List<String> displayItems = cities.stream()
                .map(c -> String.format("%s | Pop: %d | Area: %.2f | Temp: %d°C",
                        c.getName(),
                        c.getPopulation(),
                        (double) c.getArea(),
        ...
        weatherCityBox.setItems(FXCollections.observableArrayList(displayItems));
    }
